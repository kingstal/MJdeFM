...
- (void)setUserInfo:(MJUserInfo *)userInfo {
  ...
  if (!userInfo) {
    [self.loginBtn setBackgroundImage:[UIImage imageNamed:@"login"]
      ...
      setBackgroundImageForState:UIControlStateNormal
                         withURL:[NSURL URLWithString:
                                            [NSString stringWithFormat:
                                                          ...
                                                          @"icon/ul%@-1.jpg",
                                                          userInfo.userID]]
                placeholderImage:[UIImage imageNamed:@"lufei"]];
...
- (IBAction)loginBtnTapped:(UIButton *)sender {
  MJLoginViewController *loginVC =
      [[UIStoryboard storyboardWithName:@"Main" bundle:nil]
          instantiateViewControllerWithIdentifier:
              NSStringFromClass([MJLoginViewController class])];
  loginVC.delegate = self;
  [self presentViewController:loginVC animated:YES completion:nil];
}

- (void)loginViewControllerLoginSuccess:(MJLoginViewController *)loginVC
                               userInfo:(MJUserInfo *)userInfo {
