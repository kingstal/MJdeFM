...
NSString *const MJChannelViewControllerDidSelectChannelNotification =
    @"MJChannelViewControllerDidSelectChannelNotification";
...
- (void)addRecommendChannel {
  ...
  NSString *cookies =
      [MJUserInfoManager sharedUserInfoManager].userInfo.cookies;

  if (cookies) {
    [[MJFetcher sharedFetcher]
        fetchChannelWithURL:[NSString stringWithFormat:LOGINCHANNEL, cookies]
        success:^(MJFetcher *fetcher, id data) {
          NSDictionary *channelDicts = [data objectForKey:@"res"];
          for (NSDictionary *channelDict in
               [channelDicts objectForKey:@"rec_chls"]) {
            MJChannel *channel =
                [[MJChannel alloc] initWithDictionary:channelDict];
            [recommendChannels addObject:channel];
          }
          [_channels addObject:recommendChannels];
          if ([self.delegate
                  respondsToSelector:@selector(
                                         channelManagerdidUpdateChannel:)]) {
...
- (void)addTrendingChannel {
  ...
  [[MJFetcher sharedFetcher] fetchChannelWithURL:TRENDINGCHANNEL
      success:^(MJFetcher *fetcher, id data) {
        ...
        for (NSDictionary *channelDict in
             [channelDicts objectForKey:@"channels"]) {
          MJChannel *channel =
              [[MJChannel alloc] initWithDictionary:channelDict];
